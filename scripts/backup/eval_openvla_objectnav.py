...
class OpenVLAHabitatAgent(Agent):
    ...
    def act(self, observations):
        ...
        # Get the instruction from the observations
        object_idx = observations["objectgoal"]
        object_name = self.object_list[object_idx.item()]
        ...
        print(f"position: {self.position}, yaw: {self.yaw}")
...
output_path = os.path.join(
    dir_path, "outputs/eval_openvla/objectnav"
)
os.makedirs(output_path, exist_ok=True)
os.chdir(dir_path)


def load_object_list(config_path: str):
    with gzip.open(config_path, "rt") as f:
        data = json.load(f)
    objects = data['category_to_task_category_id'].keys()
    return list(objects)
...
def eval(max_steps: int = 100):
    ...
    config, dataset = load_dataset(
        "config/benchmark/nav/objectnav/objectnav_mp3d.yaml"
    )
    ...
    dataset_path = config.habitat.dataset.data_path.format(split=split)
    agent.object_list = load_object_list(dataset_path)
