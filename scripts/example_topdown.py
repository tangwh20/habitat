...
def example_pointnav_draw_target_birdseye_view():
    ...
    # plt.show()
    plt.savefig(f"{output_path}/pointnav_target_image.png")
...
def example_pointnav_draw_target_birdseye_view_agent_on_border():
    ...
    for x_edge in [-1, 0, 1]:
        for y_edge in [-1, 0, 1]:
            ...
            # plt.show()
            plt.savefig(f"{output_path}/pointnav_target_image_edge_{ii}.png")
...
def example_get_topdown_map():
    ...
    with habitat.Env(config=config, dataset=dataset) as env:
        ...
        # plt.show()
        plt.savefig(f"{output_path}/top_down_map.png")
